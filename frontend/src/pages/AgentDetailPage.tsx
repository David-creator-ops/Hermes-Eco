...
export function AgentDetailPage() {
  ...
  const resourceConfig: Record<string, { label: string; emoji: string; bg: string; installCmd: string; installPrefix: string; icon: typeof Terminal; whatItDoes: string }> = {
    ...
    skill: { label: 'Skill', emoji: '🛠️', bg: 'bg-blue-500/10 text-blue-400 border-blue-500/20', installCmd: `hermes skills install ${authorPart}${a.slug}`, installPrefix: 'hermes skills install ', icon: Code2, whatItDoes: 'A reusable procedure your agent learns and improves over time. Skills are like muscle memory for AI — once installed, the agent knows how to perform a specific task and gets better with practice.' },
    ...
    integration: { label: 'Integration', emoji: '🔌', bg: 'bg-amber-500/10 text-amber-400 border-amber-500/20', installCmd: `hermes config add integration ${authorPart}${a.slug}`, installPrefix: 'hermes config add integration ', icon: Plug, whatItDoes: 'A connector to an external service or platform. Integrations let your agent talk to APIs, databases, messaging platforms, and other services through a standard interface.' },
    workflow: { label: 'Workflow', emoji: '⚙️', bg: 'bg-rose-500/10 text-rose-400 border-rose-500/20', installCmd: `hermes cron create ${a.slug}`, installPrefix: 'hermes cron create ', icon: GitMerge, whatItDoes: 'A multi-step automation recipe that chains agents, tools, and decisions together. Workflows define how tasks flow — with branching, looping, and conditional logic for complex operations.' },
    'memory-system': { label: 'Memory', emoji: '🧠', bg: 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20', installCmd: `hermes memory add ${a.slug}`, installPrefix: 'hermes memory add ', icon: Brain, whatItDoes: 'A persistent knowledge store for your agent. Memory systems use vector embeddings and semantic search so agents can recall facts, context, and past conversations across sessions.' },
    'model-config': { label: 'Model Config', emoji: '🎯', bg: 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20', installCmd: `hermes model load ${authorPart}${a.slug}`, installPrefix: 'hermes model load ', icon: Settings, whatItDoes: 'A prompt template, personality definition, or model routing strategy. Model configs shape how your agent thinks and responds — from tone and style to which LLM handles which task.' },
    router: { label: 'Router', emoji: '🔄', bg: 'bg-orange-500/10 text-orange-400 border-orange-500/20', installCmd: `hermes config set router ${authorPart}${a.slug}`, installPrefix: 'hermes config set router ', icon: Box, whatItDoes: 'An orchestration layer that routes queries across agents and data sources. Routers decide which agent handles a task, which data source to query, and how to combine results.' },
  };
