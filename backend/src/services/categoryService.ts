...
export function getAllCategories() {
  return db.prepare('SELECT c.*, (SELECT COUNT(*) FROM agent_categories ac WHERE ac.category_id = c.id) as count FROM categories c ORDER BY c.name').all() as any[];
}

export function getCategoryBySlug(slug: string) {
  const row = db.prepare(`SELECT c.*, (SELECT COUNT(*) FROM agent_categories ac WHERE ac.category_id = c.id) as count FROM categories c WHERE c.slug = ?`).get(slug) as any;
  return row || null;
}
