...
export async function getAllCategories() {
  return await db.prepare('SELECT c.*, (SELECT COUNT(*) FROM agent_categories ac WHERE ac.category_id = c.id) as count FROM categories c ORDER BY c.name').all() as any[];
}

export async function getCategoryBySlug(slug: string) {
  const row = await db.prepare(`SELECT c.*, (SELECT COUNT(*) FROM agent_categories ac WHERE ac.category_id = c.id) as count FROM categories c WHERE c.slug = ?`).get(slug) as any;
  return row || null;
}
